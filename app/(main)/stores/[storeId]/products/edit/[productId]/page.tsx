...
function EditProductContent({ storeId, productId }: EditProductPageProps) {
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [user, storeId, productId, isAuthLoading, router, supabase]); // Added supabase to dependencies
        ...
        try {
            ...
            const { data, error } = await supabase
                ...
                .update({
                    ...
                    description: descriptionValue, // Use truncated description
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="space-y-2">
                        ...
                        />
                        <p className="text-sm text-muted-foreground">
                            최대 200자까지 입력 가능합니다. {/* Added character limit hint */}
                        </p>
                    </div>
                    ...
                    {product.is_sgt_product && (
                        <>
                            <div className="space-y-2">
                                <Label htmlFor="sgt_price_conditional">SGT 가격 (포인트)</Label> {/* Changed id to avoid conflict */}
                                <Input
                                    id="sgt_price_conditional" // Changed id to avoid conflict
                                    name="sgt_price" // Keep name same for formData
                                    value={displaySgtPrice} // Controlled by same state
                                    onChange={handleSgtPriceChange} // Controlled by same handler
