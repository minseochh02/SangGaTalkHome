  ...
  const handleSaveOptions = async () => {
    ...
    try {
      ...
      for (let i = 0; i < stateGroups.length; i++) {
        ...
        const groupPayload = {
          id: stateGroup.id, // Keep existing ID if present
          store_id: storeId,
        ...
        if (existingDbGroup) {
          ...
          groupsToUpsert.push(groupPayload);
          groupIdsToKeep.push(existingDbGroup.id);
          finalGlobalOptions.push({ ...stateGroup, id: existingDbGroup.id }); // Use existing ID
        } else {
          // New group (ID will be generated by DB or it's a new client-side ID to be replaced)
          // For new groups, we don't send the client-generated ID unless we intend to use it (which we don't for UUIDs)
          const { id, ...newGroupPayload } = groupPayload;
          groupsToUpsert.push(newGroupPayload); 
          // We will get the new ID after insert and update finalGlobalOptions
        }
      }
      
      // Upsert groups (insert new, update existing based on primary key `id`)
      // Supabase upsert handles this if `id` is provided for existing rows.
      // For new rows, `id` should be omitted to let DB generate it.
      ...
      console.log('[GlobalOptionEditor] Upserted groups:', upsertedGroupsData);

      // Map state group client IDs to new DB IDs for new groups
      let tempIdCounter = 0;
      const stateToDbIdMap = new Map<string, string>();
      upsertedGroupsData.forEach(dbGroup => {
        const originalStateGroup = stateGroups.find(sg => sg.id === dbGroup.id); // Match if ID was preserved
        if (originalStateGroup) {
          stateToDbIdMap.set(originalStateGroup.id!, dbGroup.id);
          // Update finalGlobalOptions for existing groups that were updated (ID is already correct)
          const finalOptIndex = finalGlobalOptions.findIndex(fg => fg.id === dbGroup.id);
          if (finalOptIndex !== -1) {
            finalGlobalOptions[finalOptIndex].id = dbGroup.id; 
          }
        } else {
          // This must be a NEW group that got a DB ID. We need to map it back.
          // Find the corresponding new group in state (the one without a DB ID yet)
          // This part is tricky if multiple new groups were added. Assuming order is preserved for now.
          // A robust way would be to mark new groups in state and map them sequentially.
          const newGroupInState = stateGroups.filter(sg => !sg.id || !dbGroups?.find(dbg => dbg.id === sg.id))[tempIdCounter++];
          if (newGroupInState) {
            stateToDbIdMap.set(newGroupInState.id!, dbGroup.id); // Map temporary client ID to DB ID
            finalGlobalOptions.push({ ...newGroupInState, id: dbGroup.id, choices: [...newGroupInState.choices] });
          }
        }
      });
      
      // Update the finalGlobalOptions for groups that were newly inserted and now have DB IDs
      // This ensures choices refer to correct, newly created group IDs.
      // The finalGlobalOptions should now accurately reflect the state with correct DB IDs for all groups.
      // Rebuild finalGlobalOptions ensuring all groups have their final DB ID
      const correctlyMappedFinalGlobalOptions = stateGroups.map(sg => {
        const dbId = stateToDbIdMap.get(sg.id!) || sg.id; // Use mapped DB ID or original if it was an update
        const existingFinalGroup = finalGlobalOptions.find(fg => fg.id === dbId);
        if (existingFinalGroup) return {...existingFinalGroup, choices: [...sg.choices]}; // Preserve choices from state
        return {...sg, id: dbId!, choices: [...sg.choices]};
      });
      
      // --- Process Choices for each group ---
      const choicesToUpsert: any[] = [];
      const choiceIdsToKeepByGroup = new Map<string, string[]>();

      for (const finalGroup of correctlyMappedFinalGlobalOptions) {
        if (!finalGroup.id) {
        ...
        for (let j = 0; j < stateChoicesForGroup.length; j++) {
          ...
          const choicePayload = {
            id: stateChoice.id, // Keep existing ID if present
          ...
          // Update choice ID in finalGlobalOptions after upsert if it was new
        }
      ...
      if (choicesToUpsert.length > 0) {
          const { data: upsertedChoicesData, error: upsertChoicesError } = await supabase
            .from('store_option_choices')
            .upsert(choicesToUpsert, { onConflict: 'id' })
            .select();
        if (upsertChoicesError) throw upsertChoicesError;
        console.log('[GlobalOptionEditor] Upserted choices:', upsertedChoicesData);

        // Update finalGlobalOptions with new choice IDs
        upsertedChoicesData?.forEach(dbChoice => {
            const groupForThisChoice = correctlyMappedFinalGlobalOptions.find(g => g.id === dbChoice.group_id);
            if (groupForThisChoice) {
                const choiceIndex = groupForThisChoice.choices.findIndex(c => c.name === dbChoice.name && !c.id); // Simple match for new choices
                if (choiceIndex !== -1) {
                    groupForThisChoice.choices[choiceIndex].id = dbChoice.id;
                }
            }
        });
      }
      ...
      setGlobalOptions(correctlyMappedFinalGlobalOptions); // Update state with actual DB IDs
      await fetchProductLinksForCurrentOptions(correctlyMappedFinalGlobalOptions.map(opt => opt.id || '').filter(id => id));
