...
function ProductEditorContent({ storeId, productId }: ProductEditorProps) {
	...
	// Load product data and check ownership
	useEffect(() => {
		const checkOwnershipAndLoadData = async () => {
			try {
				...
				if (store.user_id !== user.id) {
					setError(
						"해당 스토어의 소유자만 상품 상세 정보를 수정할 수 있습니다."
					);
					return;
				}
				...
				const { data: product, error: productError } = await supabase
					.from("products")
					.select("product_name, markdown_content")
				...
				if (product.markdown_content) {
					setContent(product.markdown_content);
				}
			} catch (err) {
	...
	const saveContent = async () => {
		try {
			...
			router.push(`/stores/${storeId}/products/${productId}`);
		} catch (err) {
			...
			<div className="container mx-auto py-16 px-4">
				<div className="flex justify-center items-center min-h-[50vh]">
					<div className="text-center">
						...
						<Link
							href={`/stores/${storeId}/products/${productId}`}
			...
			<div className="bg-white rounded-xl shadow-lg overflow-hidden">
				<div className="p-4 bg-gray-50 border-b">
					<h1 className="text-xl font-bold">
						상품 상세 정보 편집: {productName}
					</h1>
					<p className="text-sm text-gray-500 mt-1">
						텍스트 편집기를 사용하여 상품에 대한 풍부한 설명을 추가하세요.
						이미지, 링크, 표 등을 지원합니다.
					</p>
				</div>
...
export default async function ProductEditorPage({
	...
	return <ProductEditorContent storeId={storeId} productId={productId} />;
}
